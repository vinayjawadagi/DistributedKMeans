...
# job.name=dp.ProcessBusinesses
# job.name=dp.ProcessPopulation
# # job.name=dp.ConvertToKMeansInput
job.name=kmeans.KmeansClustering
# job.name=kmeans.KmeansParallelK
local.input=input
# local.input=inputdp
local.output=output
# Pseudo-Cluster Execution
hdfs.user.name=jerry
hdfs.input=input
hdfs.output=output
# AWS EMR Execution
...
# aws.bucket.name=cs6240-demo-bucket-jerry
aws.bucket.name=cs6240-project-jerry
...
local: jar clean-local-output
	${hadoop.root}/bin/hadoop jar ${jar.path} ${job.name} ${local.input} ${local.output} ${k} ${maxIterations}
...
aws: jar upload-app-aws delete-output-aws
	aws emr create-cluster \
		...
		--steps '[{"Args":["${job.name}","s3://${aws.bucket.name}/${aws.input}","s3://${aws.bucket.name}/${aws.output}","${k}","${maxIterations}"],"Type":"CUSTOM_JAR","Jar":"s3://${aws.bucket.name}/${jar.name}","ActionOnFailure":"TERMINATE_CLUSTER","Name":"Custom JAR"}]' \
